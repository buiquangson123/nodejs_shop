...
document.documentElement.style.setProperty(
  "--width-cate-mobile",
  $(".home__mobile-cate").offsetWidth + `px`
);
...
setTimeout(() => {
  ...
  $(".btn-next").addEventListener("click", function () {
    ...
    $(
      ".grid__row-cate-slides"
    ).style = `transform: translateX(${positionCate}px)`;
...
window.addEventListener("load", function () {
  ...
  function handleChangeFlash(number) {
    if (number === 1) {
      if (
        indexFlash * amountPro > flashLength - amountPro &&
        indexFlash == Math.floor(flashLength / amountPro)
      ) {
        if (
          postionFlashX - withItemFlash * amountPro ==
          -flashLength * withItemFlash + withItemFlash * amountPro
        ) {
          ...
          postionFlashX =
            postionFlashX -
            (flashLength - indexFlash * amountPro) * withItemFlash;
          withFlash.style = `transform: translateX(${postionFlashX}px)`;
          btnFlashNext.style.display = "none";
